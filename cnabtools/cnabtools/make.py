...
def install(duffle, skip_load, app_name, bundle_path, set, set_file):
    ...
    try:
        ...
        pass
    args = [
        'install', '-d', 'docker2', app_name,
        os.path.join(bundle_path, 'bundle.json'), '--bundle-is-file'
    ]
    for v in set:
        args += ['--set', v]
    for v in set_file:
...
def load_images(bundle_path):
    if os.path.exists(os.path.join(bundle_path, 'images.tar')):
        ...
        subprocess.run([
            'docker', 'load', '--input',
            os.path.join(bundle_path, 'images.tar')
        ],
                       check=True)
...
class Make:
    ...
    def install(self):
        parser = argparse.ArgumentParser(description='Install the application')
        parser.add_argument(
            '--skip-load',
            dest='skip_load',
            action='store_true',
        ...
        parser.add_argument(
            '--name', help='Application name', default=self.default_app_name)
        parser.add_argument(
            '--set',
            action='append',
    ...
    def uninstall(self):
        ...
        parser.add_argument(
            '--name', help='Application name', default=self.default_app_name)
        args = parser.parse_args(sys.argv[2:])
        uninstall(self._duffle(), app_name=args.name)
